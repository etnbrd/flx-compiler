...
Context.prototype.isFunction = function(id) {
  ...
  if (id.type === 'MemberExpression') {
    ...
    if (!collector.computed) {
      ...
      // var ref = h.singlify(this.currentScope.references.filter(function(ref) {
      //   return ref.identifier.name === collector.ids[0];
      // }));
      ...
      var ref = h.singlify(this.currentScope.variables.filter(function(ref) {

        // console.log(ref.name, collector.ids[0], ref.name === collector.ids[0]);

        return ref.name === collector.ids[0];
      }));
