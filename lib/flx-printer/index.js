  ...
  require('estraverse').replace(ast, {
    enter: function(n) {
      ...
      ||  n.type === 'FunctionDeclaration') {
        ...
        return {
          type: 'Identifier',
          name: arrow[n.rupturePoint.type] + ' ' + n.rupturePoint.name
        }
      }
    }
...
function signature(flx) {
  ...
  var dep = Object.keys(flx.signature).map(depType('signature'))
    .concat(Object.keys(flx.scope).map(depType('scope'))
    .concat(Object.keys(flx.sync).map(depType('sync'))))

  return ls + dep.join(', ') + rs;
}
