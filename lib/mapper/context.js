...
function Context (ast, filename, dirname) {
  ...
  // this.fluxionStartTriggers = ['app']; // TODO I am cheating here : var ex = require('express'); var app = ex() is not recognized as a fluxionTrigger.
...
Context.prototype.fluxionTrigger = function(ids, triggers) {
  ...
  // console.log("$$$ $ $ $ $$  start", ids, ids.length, this.fluxionStartTriggers.indexOf(ids[0]));
  if (ids.length === 2 && this.fluxionStartTriggers.indexOf(ids[0]) > -1 && expressRoutes.indexOf(ids[1]) > -1) {
