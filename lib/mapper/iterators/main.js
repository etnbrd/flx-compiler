...
_types.VariableDeclarator = {
  enter: function (n, p, c) {
    // TODO if we need to test n.init.callee.callee, it's because of this : require('express')(), callExpr of a callExp
    //  -> Fix this : we should'nt need to test for it.
    if (n.init.type === 'CallExpression' && n.init.callee.callee && n.init.callee.callee.name === 'require')
      c.registerPotentialFluxionTrigger(n.init.callee.arguments[0].value, n.id.name);
  }
};
