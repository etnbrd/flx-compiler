...
function codeGen(sampleType, o) {
  
  var samples = {
    declaration: function(o) {
      return [
        'var ' + o.name + ' = require(\'' + o.lib + '\')();'
      ].join('\n');
    },

    simple: function(o) {
      return [
        'var app = require(\'' + o.lib + '\')();',
  ...
  */
  ...
  describe('Fluxion Start detection', function() {
    describe('Express Library', function() {
      it('should detect the name of the express application', function() {
        var appname = makeid();
        var filename = makeid();
        var code = codeGen('declaration', {name: appname, lib: 'express'});
        var ctx = pruner(esprima.parse(code), filename);
        assert.deepEqual(ctx.fluxionStartTriggers[0], appname);
  ...
  describe('Fluxion Post detection', function() {
    describe('fs Library', function() {
      it('should detect the name of post', function() {
        var libname = makeid();
        var filename = makeid();
        var code = codeGen('declaration', {name: libname, lib: 'fs'});
        var ctx = pruner(esprima.parse(code), filename);
        assert.deepEqual(ctx.fluxionPostTriggers[0], libname);
  ...
  describe('Fluxions', function() {
    it('should be detected', function() {
      var filename = makeid(),
          flxname = makeid(),
          code = codeGen('simple', {name: flxname, lib: 'express'}),
